...
object ReflectionUtil {
    ...
    @JvmStatic
    fun findClassesFromPackage(loader: ClassLoader, classes: List<String>, packageName: String, depth: Int = 0): Classes {
    ...
    fun findMethodsByExactParameters(clazz: Class<*>, returnType: Class<*>?, vararg parameterTypes: Class<*>): List<Method> {
        return clazz.declaredMethods.filter { method ->
            if (returnType != null && returnType.name != method.returnType.name) {
                return@filter false
            }
            ...
            for (i in parameterTypes.indices) {
                if (parameterTypes[i].name != methodParameterTypes[i].name) {
                    return@filter false
                }
            }
